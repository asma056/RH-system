...
import { UpdateEquip, getEquip, postNewEquip } from '../../../../services/Stock'
...
export function FormEditEquip() {
  ...
  const [showIncorrectSpellWarning, setShowIncorrectSpellWarning] =
    ...
    useEffect(()=>{
      ...
      try {
        ...
        const response = await getEquip(
          euipId  
        )
        if (response.message === 'Successfully retrieved equipment') {
  ...
  const handleSaveSuccess = async () => {
    ...
    try {
      ...
      const response = await UpdateEquip(
        euipId ,
        name,
        description
  ...
  return (
    <EmployeesFormBody onSubmit={errorChecking}>
      <InputGroupTitle text="Update Equipement" />
      <DivInput>
            <Input
              value={name}
              type={'text'}
